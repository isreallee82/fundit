...
const Donate = (props: propsType) => {
  ...
  const handleNavigate = (props: propsType) => {
    const { datas } = props
    navigate(`/project-details/${datas.map((data) => data.name)}`, {
      state: datas,
    })
  }
  ...
  return (
    <div className='lg:p-4 grid  justify-center w-full md:p-4 '>
      ...
      <div onClick={() => handleNavigate(props)} className='md:container md:mx-auto xs:mx-2  lg:px-28 md:px-16 sm:px-16 xs:px-0 lg:my-28 md:my-28 sm:my-16 grid rounded-3xl lg:grid-cols-3 md:grid-cols-2 md:gap-8 xs:gap-4 md:justify-items-center xs:grid-cols-2 m-2 p-2'>
